            ...
            <form action="{{ route('jobs.index') }}" method="GET" class="space-y-4">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-3 pt-3 border-t border-slate-200">
                    <div class="flex flex-wrap items-center gap-2 text-xs">
                        <span class="font-bold text-slate-500">Pencarian Populer:</span>
                    ...
                    <div class="flex items-center gap-2.5 w-full sm:w-auto justify-end">
                        ...
                        @endif
                        <button type="submit" class="w-full sm:w-auto px-6 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl text-xs shadow-2xs transition flex items-center justify-center gap-2 border border-slate-900">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 mb-24">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 border-b border-slate-200 pb-4">
            ...
            <div class="flex items-center gap-2">
                <span class="px-3 py-1 bg-slate-100 text-slate-800 text-xs font-bold rounded-lg border border-slate-200">
                ...
                @foreach($jobs as $job)
                    ...
                    <div class="bg-white rounded-2xl shadow-2xs border border-slate-200 hover:border-slate-400 transition-all duration-200 flex flex-col group h-full relative overflow-hidden">
                        ...
                        <div class="p-6 flex-grow flex flex-col">
                            ...
                            <div class="mb-3">
                                <span class="inline-flex items-center text-3xs font-bold text-slate-700 uppercase tracking-wider bg-slate-100 px-2.5 py-1 rounded-md border border-slate-200">{{ $job->division }}</span>
                            </div>

                            <!-- Match Score Badge if Logged In Candidate -->
                            @if(!is_null($matchScore))
                                <div class="mb-3.5 p-2 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between text-xs">
                                    <span class="text-slate-600 font-semibold">Match Profile:</span>
                                    <span class="font-bold text-slate-900 bg-white px-2 py-0.5 rounded-md border border-slate-200">🎯 {{ $matchScore }}%</span>
                                </div>
                            @endif
                            ...
                            @if($job->quota)
                                <div class="mb-4">
                                    <div class="flex justify-between items-center text-3xs font-bold mb-1">
                                        <span class="text-slate-500">Kuota Pelamar:</span>
                                        <span class="{{ $job->isQuotaFull() ? 'text-rose-600' : 'text-slate-800' }}">{{ $job->applications()->count() }} / {{ $job->quota }}</span>
                                    </div>
                                    <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                        <div class="h-full rounded-full {{ $job->isQuotaFull() ? 'bg-rose-500' : 'bg-slate-900' }}" style="width: {{ min(100, round(($job->applications()->count() / $job->quota) * 100)) }}%"></div>
                                    </div>
                                </div>
                            @endif
                        </div>
        ...
        @else
            <div class="text-center py-20 bg-white rounded-2xl shadow-2xs border border-slate-200 max-w-2xl mx-auto">
                ...
                <a href="{{ route('jobs.index') }}" class="inline-flex items-center px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl shadow-2xs text-xs transition border border-slate-900">
